...
| SLASH-12a | **Amends SLASH-12** (`agent-forge-harness-27h`, 2026-09-20). Focus still never leaves the composer and the options are still non-focusable — that half of SLASH-12 stands. What changes: the `combobox` role moves onto a **wrapper element** around the field, carrying `aria-label`, `aria-expanded` and, while the menu is open, `aria-controls`. SLASH-12 reasoned from "a `textarea` takes no explicit role" straight to "there is no combobox"; the wrapper is the ARIA 1.1 shape, and it is the only one open to a *multiline* autocomplete. Two things forced it. (1) Nothing told assistive tech that a popup existed or whether it was open — the debounced live region was a workaround for that gap, not a replacement for it. (2) The listbox is a 40vh scroll container holding ten options with nothing focusable inside it, which axe reports as `scrollable-region-focusable`, and which was a real WCAG 2.1.1 failure: arrowing past the fold moved `aria-activedescendant` onto a row that could only be brought into view with a pointer. The companion fix is in `SlashMenu`, which now scrolls the active option into its own box — focus is never in the list, so the browser will not do it. Axe exempts a declared combobox popup from that rule. The alternative remedy, `tabindex="0"` on the listbox, was rejected: it puts the listbox in the tab order immediately before the composer, and because the menu closes on composer blur, Shift+Tab would have destroyed the element it had just moved focus to. | I |
