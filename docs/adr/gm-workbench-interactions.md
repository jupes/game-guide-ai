...
| A-15 | REVEAL-5 | Reveal ships **mask-only** in v1: the GM's Confirm is the authorisation. Eligibility binds reveal only from the Live Session Assistant's enforcement release (`1ir.11.1`), and only for a campaign whose GM has switched it on — a narrowing that stops every live display it would refuse. Confirm then gains one precondition, checked before the session row is touched (SEC-3's order). **The reveal sheet shows classes and never sets them**: classification is its own action on its own surface — a default that awaits the owner (shared ADR O-6). | shared eligibility ADR ED-9, ED-11, ED-13, M-1 to M-4, M-8 |
| A-16 | §7.1, REVEAL-7, NG-20 | The rule that a document is live in at most one slot is kept, and `1kg.7.1` expresses it as a partial unique index over live rows, so that group displays could be accepted later without a rewrite. | shared eligibility ADR ED-15 |
| A-17 | X-3, REVEAL-22 | A display write takes the campaign's authorisation row `FOR SHARE` first, validates, and only then takes the session row and slots. **No revocation asks for the authorisation lock**: a Stop takes the session row only, and an End, an expiry, a Rotate, a participant removal, a personal-link reset and an enrolment are effective under the session row and the credential they change, leaving the `authz_revision` advance to a second step under the lock — so X-3, SEC-35 and A-9 stand unchanged. Mutations that change a fact a display's preconditions read — participant add, character links, session start, archive, and later groups, classes and the enforcement switch — take the row `FOR UPDATE` first and may wait a bounded time. Every transaction holds the session row only for its write, which is all a Stop can ever wait for besides a connection. Tightening eligibility advances the reveal epoch like any other narrowing. | shared eligibility ADR ED-12, RQ-1 to RQ-11, RC-1 to RC-12 |
| A-18 | AUD-9, NG-20, NG-22, NG-25, X-2 | The four amendments the Live Session Assistant plan requests are answered: each takes its **fallback** in v1, none is rejected, and each names what is already in place for it and what a later amendment must bring. | shared eligibility ADR section 7 |
