...
| A-15 | REVEAL-5 | Reveal ships **mask-only** in v1: the GM's Confirm is the authorisation. When a campaign switches eligibility on — a narrowing that clears every slot — Confirm gains one precondition, and the sheet becomes the classification surface. | shared eligibility ADR ED-9, ED-11, M-1 to M-4 |
...
| A-17 | X-3, REVEAL-22 | A display write takes the campaign's authorisation row `FOR SHARE` first, then the session row, then slots; **a Stop takes the session row only and never waits on the authorisation lock**; tightening eligibility advances the reveal epoch like any other narrowing; participant, credential, character-link, session and rotation mutations advance `authz_revision` under that lock. | shared eligibility ADR ED-12, RQ-1 to RQ-5 |
