...
| A-18 | AUD-9, NG-20, NG-22, NG-25, X-2 | The four amendments the Live Session Assistant plan requests are answered: each takes its **fallback** in v1, none is rejected, and each names what the shared record requires of open beads now, so that accepting it later is cheaper, and what a later amendment must bring. Nothing is in place for any of them yet. | shared eligibility ADR section 7 |
