...
- **A GM-side asset**: `asset_id`, `kind` (`image` or `audio`), `state`
  (`processing`, `ready`, `failed`), the server-recorded `media_type`, size in
  bytes, `width` and `height` or `duration_ms`, the private `alt` or title, and
  timestamps. An upload answers with it; a `processing` asset has no bytes route
  yet (MS-3). The existing `AssetRef` — an id, a type, an `alt` — stays the
  reference a document field holds.
...
- **Join** answers may now carry the bounds' outcome: joined, or
  `This table is full` (RT-8), still with no reason for a failed token (SEC-8).
