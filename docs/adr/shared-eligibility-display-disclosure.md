...
| ED-9 | **A reveal Confirm is one transaction whose preconditions are a named, ordered list, in the threat model's order (SEC-3).** Authentication, role and ownership (404) are the route's and come first. Then, under the campaign's authorisation lock in share mode (RQ-4) and **before the session row is touched**, the validation that depends on the resource — each a 422: (1) the named version is sealed and is the one the sheet displayed (REVEAL-5, CANVAS-34); (2) every masked key is on the type's revealable allowlist and is present and non-empty in that version (REVEAL-5, REVEAL-9, REVEAL-10); (3) the audience is one the type allows and, for a participant, names an active participant linked as ED-14 requires; (4) **when Enforced**, every masked key is eligible for the slot's audience (ED-10). Then state — a 409: (5) the table session row is locked, the session is live and its reveal epoch is the one the Confirm carries (REVEAL-22). Then the write, under the one-slot rule (ED-15). The first failing precondition refuses the whole Confirm; nothing is displayed in part. | W + T (SEC-3) + L |
...
| ED-12 | **Narrowing always wins (X-3), and anything that shrinks the set of participants a live display's keys are eligible for is a narrowing**: a tightened class, a character unlink or relink, a removal from a group, a removed participant. In the transaction that makes the change effective it stops every live display holding a key that is no longer eligible for its slot's audience — the whole display, not the one field — writes `stopped` events and **advances the Workbench reveal epoch**, so a widening the GM staged before it is a 409 (REVEAL-22). Under the campaign lock — the same transaction for a locked mutation, the second step for a revocation (RQ-5) — it removes the affected table-namespace rows (L §4.3 rule 4) and advances `authz_revision`. Any class change that removes a participant advances the epoch, which is also what makes a reveal sheet drawn before it harmless. | W + L §4.3, §4.9 |
...
| ED-21 | **Exports use the same builder and the same rule.** A player-safe export contains the subset the GM ticks in the staged dialog (EXPORT-3), every key of which must be on the revealable allowlist and — when Enforced — `public`: a file can be forwarded to anyone, so no narrower class can be honoured once it has left. The dialog previews the exact text, **never classifies**, and from `1ir.2.8` writes one `exported` ledger row per field (L P10). Print is the same projection (EXPORT-7). | W (EXPORT-3, EXPORT-7, REVEAL-21) + L |
...
eligible_for_audience(document, key, audience) when Enforced      -- identities, never credentials
  ...
  participant <id> is not active in this campaign -> no
  campaign                                        -> yes
...
entitled(requester, slot)                                          -- credentials, at every table read
  ...
  slot is table                                   -> yes
  slot is participant:<id>                        -> the device credential is valid and names <id> (AUD-5)
...
display(slot, document, version, mask) - one transaction (ED-9)
  ...
  validate, reading without row locks:
    the version is sealed and is the one the sheet displayed                         else 422
  ...
  lock the table session row; require it live and its reveal epoch = the Confirm's   else 409
...
stop(slot or all) - never refused, never takes the campaign lock (RQ-6)
  lock the table session row only
  clear the slot if it holds what the Stop names; advance the epoch even if it was empty
  write stopped (or retracted); notify

credential or session change - End, expiry, Rotate, Remove participant, Reset personal link,
                               device enrolment and replacement - two steps (RQ-5)
  step 1, never takes the campaign lock:
    lock the participant or credential row being changed, then the session row if one is live
    make the change true: end the session, advance the link generation, revoke the credential
    clear the slots it affects; advance the epoch if it narrows; write events
    enqueue the bookkeeping job; commit; notify
  step 2, the job - run at once after the commit and retried until done:
    lock_campaign(exclusive); clear any slot step 1 could not have seen (RC-6);
    run the table-namespace narrowing when there is one; advance authz_revision

locked mutation - participant add; character link, unlink and relink; session start; document archive
                  and delete; campaign deletion; and, when they exist: group membership, classification,
                  version approval, the enforcement switch - one transaction (RQ-4)
  lock_campaign(exclusive) first; change the fact
  if it narrows: scan, then under the session row stop what is no longer allowed and advance the epoch (ED-12)
  advance authz_revision
```
...
| ID | Step | Basis |
| --- | --- | --- |
| M-1 | **Workbench v1 builds reveal exactly as its record says** (sections 7 and 8, threat model SEC-13 to SEC-19), with no eligibility table and no eligibility check, and with what section 9 requires from day one: the campaign authorisation row, its helper and its lock order; `authz_revision`; two-step credential and session changes; content-free audit rows (ED-18); and a Confirm whose preconditions are a named, ordered list (ED-9). | ED-11 |
| M-2 | **Enforcement arrives with `1ir.11.1` (Phase 4), per campaign, by its own GM action** — never silently by a deploy, and **not** as a side effect of the capture policy's `enabled` (L §3.1), which is a reversible toggle about listening. The flag lives on the campaign's `authz_state` row, so the transaction that locks the row has read it. The classification surface exists from Phase 2 (`1ir.2.7`), so a GM can classify before switching on. **The assistant's display features — live-card shares, the reveal hand-off, rule re-displays, recap publication — require the campaign to be Enforced.** Until then a campaign behaves as v1. | L + I · **E** (O-7) |
| M-3 | **Switching it on is a narrowing.** It is a locked mutation (RQ-4): under the campaign lock it runs ED-12's scan over the campaign's live slots, stops every display holding a key that is not eligible for its audience with `stopped(enforcement_enabled)`, and advances the epoch. The GM is told first what will stop; the simplest path is to switch it on between sessions. | X-3 + I |
...
| RQ-2 | **One mechanism, one helper.** The campaign lock is a **row lock on `campaign.authz_state`**, `FOR SHARE` or `FOR UPDATE`, as L §4.3 has it. No advisory lock stands in for it: the existing `UnitOfWork.lock` is an exclusive advisory lock and is not this, and two code paths using two mechanisms would not exclude each other at all. `1kg.2.1` adds `lock_campaign(campaign_id, *, shared)` to the unit of work in `service/db.py`, with an in-memory twin that records the campaign and the mode. The helper (a) is the first lock its transaction takes — the twin fails a test that takes another lock through the unit of work first; (b) sets `lock_timeout` and `transaction_timeout` for the transaction (RQ-8) with `set_config(..., true)`; (c) **raises if the row is missing** — fail closed, never proceed unlocked; (d) requires READ COMMITTED, the default, so that every check made after the lock reads what the last holder committed. Under REPEATABLE READ a waiter is safe only because every exclusive holder updates the row, and no caller may rely on that. | `1kg.2.1` |
| RQ-3 | **One lock order, everywhere: the campaign's `authz_state` row; then participant, character-link and document rows, if the transaction must lock any; then the table session row; then slot rows.** A transaction may skip levels and never goes back up. No transaction takes two campaigns' locks: work that spans two campaigns — ending a session in one to start a session in another — is two transactions, the End first. A Confirm locks no document or participant row: it reads them under the share lock, and archive, unlink and removal are covered by the campaign lock and the epoch. Connection admission's per-session advisory lock (RT-8) is taken alone: nothing that holds a lock in this order asks for it, and admission asks for none of these. | `1kg.2.2`, `1kg.2.3`, `1kg.5.2`, `1kg.7.2`, `1kg.7.5`, `1ir.2.x` |
| RQ-4 | **Display writes take the campaign lock in share mode, then check** — Reveal, Update, replace and move, and later Share, the reveal hand-off, rule re-displays and recap publication. **Locked mutations take it exclusively, first, and do everything in that one transaction**: participant add; character link, unlink and relink; session start; document archive and delete; campaign deletion; and, when they exist, group membership, classification, version approval and the enforcement switch. Each changes a fact that a display's preconditions read, so a display either commits before the mutation, which sees it and stops it, or runs after it and is refused (L §4.3). A locked mutation may wait, within RQ-8's bound; the control that never waits is Stop. | `1kg.2.2`, `1kg.2.3`, `1kg.5.2`, `1kg.2.6`, `1kg.7.2`, `1ir.2.x`, `1ir.11.1` |
| RQ-5 | **Credential and session changes never wait for the campaign lock.** End, expiry, Rotate, Remove participant, Reset personal link, and device enrolment and replacement are effective in a first transaction that locks the participant or credential row it changes and the session row if one is live — never `authz_state`. That transaction makes the change true for every reader at once (ED-25: the session ended, the link generation advanced, the credential revoked — SEC-9, SEC-11), clears the slots it affects, advances the reveal epoch if it narrows (REVEAL-22), writes its audit rows and enqueues one content-free bookkeeping job on the outbox of `1kg.1.5`, deduplicated per campaign. The job runs at once after the commit and is retried until done: under the campaign lock, exclusively, it clears any slot the first step could not have seen (RC-6), runs the table-namespace narrowing when there is one (L §4.3 rule 4), and **advances `authz_revision`**. X-3, SEC-35 and A-9 therefore stand unchanged: an End and a Rotate are never queued behind a projector and never refused by a lock timeout, and neither is the remedy for an intercepted personal link. An unauthenticated enrolment verifies its code before it takes any lock. This is the one place this record departs from the letter of L §4.9's "same transaction": the revocation and its slot clears are one transaction, and the revision follows it. | `1kg.2.2`, `1kg.2.3`, `1ir.2.1`, `1ir.11.1` |
| RQ-6 | **A Stop takes the table session row and slot rows, and nothing else** — never `authz_state` — so it cannot deadlock against RQ-3 and never waits for a projector or a locked mutation. **What it can wait for is said plainly.** (a) The session row, held by one other reveal-path transaction at a time: so every transaction holds the session row **only for its write**. Validation, projection building, scans of other tables and table-namespace rewrites happen before it is taken (ED-9); what happens under it is an epoch check, slot writes, an epoch advance, event rows and the commit. (b) A database connection: a request blocked on the campaign lock holds one of its instance's gate slots (`DB_POOL_MAX`, 4), so RQ-8 keeps that wait well under the gate's 5 s, and `1kg.7.2` must show that a Stop is served while every other slot is held by a lock waiter — by that bound, or by a slot reserved for narrowings, with the connection budget of `docs/migrations.md` re-proved if it changes. A Stop and any other narrowing that race are both narrowings and are idempotent. | `1kg.7.2` |
| RQ-7 | **Every narrowing advances the session's reveal epoch, on an empty slot too** (REVEAL-22) — every shrink of ED-12 and the enforcement switch (M-3) included. `1kg.7.1` exposes one function that stops named slots and advances the epoch inside the caller's transaction, under the session row. Its callers are `1kg.7.2` (Stop, replace, move), `1kg.2.2` (unlink, remove, reset), `1kg.2.3` (End, expiry, Rotate), `1kg.5.2` (archive, delete), `1kg.2.6` (campaign deletion) and, when they exist, `1ir.2.1` and `1ir.2.7` (groups and classification) and `1ir.11.1` (the switch and the scan). The resolver (`1ir.2.2`) mutates nothing and calls nothing. | `1kg.7.1` and its callers |
| RQ-8 | **Lock waits and lock holds are bounded.** No network or model call is made under `authz_state` (L §4.3 rule 7). The helper sets `lock_timeout` (*suggested* 2 s, always under the gate's acquire timeout) and **`transaction_timeout`** (*suggested* 5 s; PostgreSQL 17) — a bound on the *transaction*, which `statement_timeout` is not. It ends the session, which per-operation connections tolerate, and a database test must prove that it arms when set inside the transaction. **A narrowing that times out waiting is retried by the server — within the request, then by a job — and is never reported to the GM as refused**; a widening that times out is a retryable `503`. The projector (`1ir.2.3`) holds the lock in **short slices** — a bounded number of queue items per transaction rather than every pending item, setting `projection_revision` only in the slice that finds the queue empty — and gives way between slices. PostgreSQL does not promise an exclusive waiter a turn ahead of later share-mode lockers of a row, so nothing here relies on fairness: share-mode traffic is one GM's displays, each bounded by `transaction_timeout`, and `1ir.11.2`'s send-time re-checks must not become a steady stream (RC-8). | `1kg.2.1`, `1ir.2.3`, `1ir.11.2` |
...
| RC-2 | A Confirm and a locked narrowing — unlink, tighten, group removal, archive — in either order | the Confirm commits first and the narrowing's scan stops it, or the narrowing commits first and the Confirm is refused | share against exclusive on one row, under READ COMMITTED (RQ-2, RQ-4) | `1kg.7.2`, `1ir.11.1` |
...
| RC-5 | A Confirm and an End or a Rotate | End or Rotate first: a 409 — no live session, or a stale epoch. Confirm first: its slot is cleared | the session row (RQ-5) | `1kg.7.2`, `1kg.2.3` |
| RC-6 | Remove participant while **no** session is live, racing a session start and a Confirm to that participant | the Confirm may commit to a slot nobody can read — the credential is already revoked (ED-25) — and the removal's second step clears it | RQ-5 | `1kg.2.2` |
| RC-7 | A Stop while a locked narrowing is in flight | the Stop waits at most for that transaction's session-row write — never for its scans or table-namespace rewrites, never for `authz_state` | RQ-6 | `1kg.7.2`, `1ir.11.1` |
...
| RC-11 | Two locked mutations | serialised by the lock; the second reads what the first committed | RQ-4 | `1kg.2.2` |
...
| `1kg.2.1` | RQ-1, RQ-2, RQ-10: the `campaign` schema; `authz_state` with its trigger and its lock-only column; `lock_campaign` and the revision helper in `service/db.py`, with in-memory twins; ED-18(a): the audit table, unless `1ir.2.6` merged first; participants, credentials, character links and sessions shaped so that RQ-5's first step needs no lock on `authz_state` |
| `1kg.2.2` | RQ-3, RQ-4, RQ-5, RQ-7: participant add and character link, unlink and relink are locked mutations; Remove participant, Reset personal link and device enrolment and replacement are two-step and never take the lock in their first step; enrolment verifies its code before any lock; every narrowing calls `1kg.7.1`'s stop-and-advance function; RC-6, RC-11, RC-12 as tests |
| `1kg.2.3` | RQ-3, RQ-4, RQ-5, RQ-7, RQ-11: session start is a locked mutation; End, expiry and Rotate are two-step, effective under the session row alone, and advance the link generation and both epochs there (SEC-9); RC-5, RC-8 as tests |
| `1kg.2.6` | campaign deletion is a locked mutation: it takes the lock exclusively, stops what is live, then deletes (SEC-36); `authz_state` goes with the campaign |
...
| `1kg.5.2` | archive and delete of a document are locked mutations and call the stop-and-advance function (LIB-17, RQ-4, RQ-7); the step that migrates a document to a newer type version follows ED-24 |
| `1kg.1.6` | The reveal family of the wire contract: masks as explicit key lists; audiences `table` and `participant:<id>`; a slot content `content_kind` whose only member is `document` (7.4); a 422 for a mask that lists the keys at fault as `keys`, never text; no eligibility fields in v1 — the eligibility refusal is an additive code later (section 4) |
| `1kg.7.1` | ED-15 (the partial unique index); ED-18(a) (the audit rows of a reveal, exactly those fields); ED-26 (no hash); RQ-7 (the stop-and-advance function) |
| `1kg.7.2` | ED-9 and RQ-9 (ordered preconditions; validation and projection building before the session row); ED-16; ED-25; RQ-3; RQ-4 (share mode); RQ-6 (Stop); RQ-8; section 5's v1 column and RC-1, RC-2, RC-4, RC-5, RC-7, RC-9 as tests |
| `1kg.7.3` | REVEAL-4's seeding unchanged; in v1 no class is shown and nothing classifies (M-4) |
...
| `1ir.2.1` | ED-4, ED-7, ED-8, ED-24; `field_path` is the flat key of ED-2; RQ-10: it *extends* the Workbench's helper with rule 3 and tolerates the previous release's helper; RQ-5: credential and session changes advance the revision in a second step |
| `1ir.2.2` | section 4's two functions — `eligible_for_audience` on identities, `entitled` on credentials (ED-10, ED-25); the per-request re-read of session, generation, credential and participant status (RQ-11); section 5's Enforced column as acceptance |
...
| `1ir.2.6` | ED-18(a): one audit table, shared with the Workbench, whichever epic creates it; ED-26 |
| `1ir.2.7` | The classification surface (ED-13, M-4, M-6): the only place a class is set; keys off the revealable allowlist are not counted as unclassified (ED-5); any change that removes a participant is a narrowing (ED-12) |
| `1ir.2.8` | ED-17, ED-18(b), ED-26: per-field events with reasons, `exported` included; no backfill; keyed or reference-only hashes |
| `1ir.5.1` | ED-23: aliases are a second subject kind in the one model |
| `1ir.11.1` | The enforcement release: ED-9's fourth precondition, ED-10, ED-12's scan, M-2, M-3, M-4, M-8, the enforcement flag on `authz_state`, the additive refusal code, the sheet's class display (with a follow-up to `1kg.7.3`'s component), RC-2, RC-3, RC-7; section 5's Enforced column as acceptance |
| `1ir.11.2` | RQ-8: send-time re-checks are short and must not form a steady stream of share-mode holders (RC-8); RQ-11 |
| `1ir.1.12` | M-7 |
| `1ir.1.13` | RQ-1: a display role needs `UPDATE (lock_token)` on `authz_state` and nothing else there; an authorisation role `UPDATE` on the row; a Stop, the first step of a credential or session change and the enrolment handler need no right on it; the trigger that creates the row must work under whatever role creates campaigns |
...
| X-3 | Adopted and extended: tightening eligibility is a narrowing (ED-12) and switching enforcement on is a narrowing (M-3). Kept whole for the controls X-3, SEC-35 and A-9 name: a Stop, an End and a Rotate never ask for the authorisation lock (RQ-5, RQ-6). What a Stop can still wait for — one short session-row write, and a connection — is stated in RQ-6 |
...
| When | 2026-09-19 |
| Accepted for the Workbench | ED-1 to ED-26 as the Workbench's reading of the shared model; M-1; RQ-1 to RQ-11 and RC-1 to RC-12 as requirements on its beads; rows A-14 to A-18 of the Workbench record |
| Not accepted on the owner's behalf | O-1 to O-7. Each is a change of product scope with a default in force: **escalated, not decided**, and tracked in `agent-forge-harness-1kg.1.7` |
| Not yet seen by | the owner; and the Live Session Assistant's planning session, whose plan (PR #60) this record departs from in RQ-5 (two-step revocations), RQ-8 (projector slices, `transaction_timeout`), ED-18 (which bead creates the audit table), ED-26 (hashes) and section 4 (delivery reads state) |
| Reviewed by | one independent adversarial review; every finding is answered in section 15 |
...
| V-3 | Medium | Classify-and-display in one Confirm contradicted section 2 and section 4, over-widened, needed a lock upgrade and had no staleness check | **Fixed by reversing the default.** The reveal sheet never classifies (ED-13, M-4); the alternative is O-6, with what it would have to bring. A narrowing class change advances the epoch, so a stale sheet is a 409 (ED-12). A-15 rewritten |
